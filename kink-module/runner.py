...
class Kink:
	...
	clip_argument_lists = []
	movie_argument_lists = []
	trailer_argument_lists = []
	...
	def __init__(
		...
		Kink.clip_dispatcher_thread = threading.Thread(
			target=self.__dispatch_clips)
		Kink.photo_dispatcher_thread = threading.Thread(
			target=self.__dispatch_movie)
		Kink.clip_dispatcher_thread = threading.Thread(
			target=self.__dispatch_trailer)
	...
	def __dispatch_photos(self):
		...
		while not Kink.stop_dispatching or len(Kink.photo_argument_lists) != 0:
			...
			pool.map(self.__download_media, aria_command_list)
			pool.map(self.__unzip_media, _7z_command_list)
			...
			photo_rename_command = self.__build_photo_rename_command(
				photo_dir, id)
			subprocess.run(photo_rename_command)

			self.__suction_photos(photo_dir)
				
		print("Exiting photo dispatcher thread...")

	def __dispatch_clips(self):
		print("Beginning clip dispatcher thread...")
		while not Kink.stop_dispatching or len(Kink.clip_argument_lists) != 0:
			if len(Kink.clip_argument_lists) == 0:
				continue

			print("Argument list found! Dispatching...")
				
		print("Exiting clip dispatcher thread...")

	def __dispatch_movie(self):
		print("Beginning movie dispatcher thread...")
		while not Kink.stop_dispatching or len(Kink.movie_argument_lists) != 0:
			if len(Kink.movie_argument_lists) == 0:
				continue
	...
	def __dispatch_trailer(self):
		print("Beginning trailer dispatcher thread...")
		while not Kink.stop_dispatching or len(Kink.trailer_argument_lists) != 0:
			if len(Kink.trailer_argument_lists) == 0:
				continue
	...
	def __save_photos(self, site, title):
		...
		for i in range(0, len(zips)):
			aria_command_handle = self.__build_photo_zip_aria_command(
				zips, i, dir_name, error_strings, site, title)
			_7z_command_handle = self.__build_photo_zip_7z_command(
				zips, i, dir_name, error_strings, site, title)
	...
	def __build_photo_zip_aria_command(self, zips, i, dir_name, error_strings, 
		site, title):
	...
	def __build_photo_zip_7z_command(self, zips, i, dir_name, error_strings, 
		site, title):
	...
	def __build_photo_rename_command(self, dir, id):
	...
	def __suction_photos(self, dir, is_prime_invocation=True):
		dirs = []
		files = []
		...
		if len(dirs) > 0:
			for d in dirs:
				files += self.__suction_photos(d, False)
